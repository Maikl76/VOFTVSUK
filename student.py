...
def delete_student(student_id):
    try:
        response = supabase.table("students").delete().eq("id_op", student_id).execute()
        return response.data
    except Exception as e:
...
def run_edit_student():
    ...
    with st.form(key="edit_student_form"):
        new_hodnost    = st.selectbox("Hodnost", ["--","svob.","des.","čet.","rtn. Bc.","rtm. Bc."],
                                      index=["--","svob.","des.","čet.","rtn. Bc.","rtm. Bc."].index(selected_student.get("hodnost","--")),
                                      key="edit_hodnost")
        ...
        dob_default    = datetime.datetime.strptime(selected_student.get("date_of_birth","1970-01-01"), "%Y-%m-%d")
    ...
    if confirm_delete and st.button("Smazat studenta", key="delete_student_edit"):
        delete_student(selected_student.get("id_op"))
