...
def run_pri(selected_year):
    ...
    for rok in [str(current_year), str(next_year)]:
        ...
        if df.empty:
            ...
            # Editace (pokud není podporován inline editor)
            if hasattr(st, "experimental_data_editor"):
                edited_df = st.experimental_data_editor(
                    df, num_rows="dynamic", key=f"pri_editor_{rok}", use_container_width=True
                )
                if st.button("💾 Uložit změny", key=f"pri_save_{rok}"):
