...
def run_zsc():
    ...
    with st.form("zsc_add_form", clear_on_submit=True):
        ...
        submitted_add = st.form_submit_button("➕ Přidat cestu")
        if submitted_add:
    ...
    if df.empty:
        ...
        with st.form(f"zsc_edit_form_{idx_edit}", clear_on_submit=True):
            ...
            submitted = st.form_submit_button("💾 Uložit změny", key=f"zsc_submit_{idx_edit}")
            if submitted:
    ...
    # Finance, historie, archiv remain unchanged  
